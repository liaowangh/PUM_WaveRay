...
void pum_resolution_test(const std::string& mesh_path, bool hole_exist, size_type L, double wave_nr, 
...
//     // auto mesh_path = (here.parent_path().parent_path() / ("meshes/square.msh")).string(); 
    ...
    for(auto k: wave_number) {
        ...
        for(int i = 0; i < solutions.size(); ++i) {
            ...
            // pum_resolution_test(square_hole, true, L, k, number_waves, square_hole_output, solutions[i], str);
            pum_resolution_test(square, false, L, k, number_waves, square_output, solutions[i], str);
            std::cout << std::endl;
        }
    }
}
