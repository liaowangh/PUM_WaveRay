...
double power_wave_ray(HE_LagrangeO1& he_O1, ExtendPUM_WaveRay& epum, int wave_start, int wave_coarselayers,
    int ray_start, int ray_coarselayers, double k, bool verbose = false) {
    ...
    while(true) {
        cnt++;
        old_v = v;
        pum_wave_cycle(v, zero_vec, wave_op, ray_op, wave_I, ray_I, k, stride, 
            wave_ms, ray_ms, wave_start, ray_start);        
