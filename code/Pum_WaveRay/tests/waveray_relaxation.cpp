...
void relaxation_factor(PUM_WaveRay& pum_waveray, int L, int num_wavelayer) {
    ...
    for(int i = num_wavelayer - 1; i >= 0; --i) {
        ...
        stride[i] = pum_waveray.num_planwaves[idx] + 1;
        // stride[i] = 1;
    }
...
int main(){
    ...
    int num_wavelayer = 5;
    relaxation_factor(pum_waveray, L, num_wavelayer);    
}